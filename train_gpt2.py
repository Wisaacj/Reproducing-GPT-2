...
class DataLoaderLite:
    ...
    def __init__(self, B: int, T: int, process_rank: int, num_processes: int, split):
        self.B = B
        self.T = T
...
# Determining the number of gradient accumulation steps
...
model.to(device)
use_compile = False  # torch.compile interferes with HellaSwag eval and Generation FIXME
if use_compile:
...
for step in range(max_steps):
    ...
    if (step % 250 == 0 or last_step) and (not use_compile):
        ...
        for i, example in enumerate(iterate_examples("val")):
            ...
            with torch.no_grad():
                ...
                pred_norm, _, _, _ = get_most_likely_row(tokens, mask, logits)
            num_total += 1
        ...
        while xgen.size(1) < max_length:
            ...
            with torch.no_grad():
                ...
                ix = torch.multinomial(
                    topk_probs, num_samples=1, generator=sample_rng)  # (B, 1)
    ...
    # Training: do one step of optimisation.
    model.train()
    optimizer.zero_grad()
    loss_accum = 0.0
    ...
    if master_process:
        print(f"""step {step:5d} | loss: {loss_accum:.6f} | lr: {lr:.4e} | norm: {norm:.4f} | dt: {
